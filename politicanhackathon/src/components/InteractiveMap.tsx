...
export default function InteractiveMap() {
  ...
  const getStateData = (stateCode: string): StateData => {
    ...
    // Mock governor data - in a real app, this would come from an API
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="grid grid-cols-4 gap-2">
              {['CA', 'TX', 'NY', 'FL', 'IL', 'OH', 'WA', 'NV'].map((stateCode) => (
                <div
                  key={stateCode}
                  className={`
                    p-4 rounded-lg border-2 cursor-pointer transition-all duration-200
                    ${selectedState === stateCode ? 'border-blue-500 shadow-lg' : 'border-gray-200 hover:border-gray-300'}
                  `}
                  style={{ backgroundColor: getStateColor(stateCode) }}
